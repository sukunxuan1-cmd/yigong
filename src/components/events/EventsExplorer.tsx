...
export default function EventsExplorer({ events }: { events: VolunteerEvent[] }) {
  ...
  const chip = (on: boolean) =>
    `rounded-full px-3.5 py-1.5 text-xs font-semibold transition-colors ${
      on ? "bg-gradient-to-r from-leaf to-mint text-ink" : "border border-white/12 text-slate-300 hover:border-leaf/50"
    }`;
  ...
  return (
    <>
      ...
      <div className="mx-auto mt-8 w-[min(94%,72rem)]">
        <div className="glass rounded-3xl p-4">
          <div className="flex flex-col gap-3 md:flex-row md:items-center">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full rounded-2xl border border-white/10 bg-white/5 py-2.5 pl-11 pr-4 text-sm text-white placeholder:text-slate-500 focus:border-leaf/60 focus:outline-none"
              />
            </div>
            {active && (
              <button
                ...
                className="shrink-0 rounded-2xl border border-white/10 px-4 py-2.5 text-xs text-slate-300 hover:text-white"
              >
                清除筛选 ✕
              </button>
            )}
          ...
          <div className="mt-3 flex flex-wrap items-center gap-2">
            ...
            <span className="ml-3 text-xs text-slate-500">年份</span>
            {allYears.map((y) => (
      ...
      {filtered.length > 0 ? (
        ...
        <div className="py-32 text-center text-slate-500">没有匹配的活动，换个关键词试试～</div>
      )}
    </>
  );
}
